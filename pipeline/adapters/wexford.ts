...
// Wexford County Council publishes its register only as a PDF (no coordinates).
// pipeline/manual/wexford_convert.ts scrapes it into this CSV (owner column
// dropped); this adapter reads it and leaves lat/lon null so geocode.ts places
// each site from its address.
const CSV_PATH = "data/manual/wexford.csv";
const SOURCE_URL = "https://www.wexfordcoco.ie/planning/building-control/derelict-and-vacant-sites";
const COUNCIL = "Wexford";
// The date we converted the PDF. Bump when re-running wexford_convert.ts.
const RETRIEVED = "2026-07-11";

// "DER2024-060" -> "der2024-060" for a clean id.
...
export async function load(): Promise<Site[]> {
  ...
  return rows.map((row) => {
    ...
    let address = row.address?.trim() || (ref ? `Wexford, register ref ${ref}` : "Wexford");
    if (!/\bwexford\b/i.test(address)) address += ", Co. Wexford";

    const valuation = Number(row.valuation);

    return makeSite({
      id: ref ? `wexford-${slug(ref)}` : `wexford-${slug(address)}`,
      council: COUNCIL,
      address,
      register_ref: ref,
...
// Run directly:  npx tsx pipeline/adapters/wexford.ts
